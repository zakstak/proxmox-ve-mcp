...
export function registerContainerReadTools(
  ...
  server.tool(
    ...
    {
      node: z.string().regex(/^[a-zA-Z0-9_-]+$/, 'Invalid node name').optional().describe('Node name (omit to list across all nodes)'),
    },
    async ({ node }) => {
      try {
        if (node) {
...
export function registerContainerWriteTools(
  ...
  server.tool(
    ...
    'Clone an existing container to create a new one',
    {
      ...
      hostname: z.string().regex(/^[a-zA-Z0-9_.-]+$/, 'Invalid hostname').optional().describe('Hostname for the new container'),
      node: z.string().regex(/^[a-zA-Z0-9_-]+$/, 'Invalid node name').optional().describe('Node name'),
