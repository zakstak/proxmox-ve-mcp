...
export function registerTaskReadTools(
  ...
  server.tool(
    ...
    {
      upid: z.string().regex(/^[a-zA-Z0-9_:@-]+$/, 'Invalid UPID format').describe('Task UPID (unique task ID)'),
      node: z.string().regex(/^[a-zA-Z0-9_-]+$/, 'Invalid node name').optional().describe('Node name'),
    },
    async ({ upid, node }) => {
      try {
...
export function registerTaskWriteTools(
  ...
  server.tool(
    ...
    {
      upid: z.string().regex(/^[a-zA-Z0-9_:@-]+$/, 'Invalid UPID format').describe('Task UPID'),
      node: z.string().regex(/^[a-zA-Z0-9_-]+$/, 'Invalid node name').optional().describe('Node name'),
    },
    async ({ upid, node }) => {
      try {
